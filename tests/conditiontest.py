...
class SimpleTest3(simpletest.SimpleTest):
    def setUp(self):
        ...
        lines.create()
        mapper(Block, blocks)
        c = a.Source(lines.c.block) == a.Target(blocks.c.id)
        mapper(Line, lines,
        ...
        self.tagging_mapper = mapper(Tagging, tagging,
            extension=self.aggregator_class(
                a.Count(movies.c.tag_count,
                    (a.Target(movies.c.id) == a.Source(tagging.c.object_id)) & (a.Source(tagging.c.tbl) == 'movies')),
                a.Count(events.c.tag_count,
                    (a.Target(events.c.id) == a.Source(tagging.c.object_id)) & (a.Source(tagging.c.tbl) == 'events')),
                ))
...
class TestUserpics(testbase.TestBase):
    def setUp(self):
        ...
        self.userpic_mapper = mapper(Userpic, userpics,
            extension = self.aggregator_class(
                a.Count(users.c.pic_count, (a.Target(users.c.id) == a.Source(userpics.c.uid)) & (a.Source(userpics.c.state) == "normal")),
                ))
...
class TestBlog(testbase.TestBase):
    def setUp(self):
        ...
        self.blog_mapper = mapper(BlogEntry, blog,
            extension=self.aggregator_class(
                a.Count(stats.c.posts_so_far, a.Target(stats.c.date) >= a.Source(blog.c.date)),
            ))
